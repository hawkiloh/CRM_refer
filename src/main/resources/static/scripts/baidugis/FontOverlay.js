/**
 * @fileoverview 此类表示地图上的一个覆盖物，该覆盖物由字体组成，从Overlay继承。
 * 主入口类是<a href="symbols/BMapLib.FontOverlay.html">FontOverlay</a>，
 * 基于Baidu Map API 1.2。
 *
...
(function () {
    ...
    /**
     * @exports FontOverlay as BMapLib.FontOverlay
     */
    var FontOverlay =
        /**
         * FontOverlay
         *
         * @class 此类表示地图上的一个覆盖物，该覆盖物由字体组成，从Overlay继承。 该覆盖物适用于以下类似的场景：需要在地图上添加一系列覆盖物，这些覆盖物之间用不同的字体来区分，动匹配相应颜色和大小的图标。
         *
         * @constructor
         * @param {Point}
         *            position 表示一个经纬度坐标位置。
         * @param {Json
         *            Object} options 可选参数，可选项包括：<br />
         *            className {String} 图片的url地址。(可选)<br />
         *            fontSize {Size} 表示字体大小。（可选）<br />
         *            fontColor {String} 表示字体颜色。（可选）<br />
         */
        BMapLib.FontOverlay = function (position, options) {
            this._position = position;
            this._options = options || {};
            this._className = this._options['className'] || 'icon-circle';
            this._fontSize = this._options['fontSize'] || 24;
            this._fontColor = this._options['fontColor'] || 'red';
        };

    T.lang.inherits(FontOverlay, BMap.Overlay, "FontOverlay");
     ...
     */
    FontOverlay.prototype.initialize = function (map) {
        this._map = map;
        var div = this._domElement = document.createElement('div');
        div.style.position = "absolute";
        div.style.cursor = "pointer";
        div.style.zIndex = BMap.Overlay.getZIndex(this._position.lat);
        div.style.color = this._fontColor;
        div.style.fontSize = this._fontSize + "px";
        var i = this._i = document.createElement("i");
        i.className = this._className;
        div.appendChild(i);
        this._bind();
        this._map.getPanes().labelPane.appendChild(this._domElement);
     ...
     * @return 无返回值。
     */
    FontOverlay.prototype.draw = function () {
     ...
     */
    FontOverlay.prototype.getPosition = function () {
     ...
     * @return 无返回值。
     */
    FontOverlay.prototype.setPosition = function (position) {
     ...
     * @return 无返回值。
     */
    FontOverlay.prototype._updatePosition = function () {
        if (this._domElement && this._position) {
            ...
            pixelPosition.x -= Math.ceil(parseInt(style.fontSize) / 2);
            pixelPosition.y -= Math.ceil(parseInt(style.fontSize) / 2);
     ...
     * 当鼠标点击该覆盖物时会触发该事件
     *
     * @name FontOverlay#click
     *
     * @event
     *
     * @param {Event
     *            Object} e 回调函数会返回event参数，包括以下返回值：
     *
     * <br />"<b>type</b> : {String} 事件类型
     *
     * <br />"<b>target</b>：{BMapLib.FontOverlay} 事件目标
     *
     *
     *
     */
     ...
     * @example <b>参考示例：</b><br />
     *
     * myFontOverlay.addEventListener("mouseover", function(e) { alert(e.point); });
     *
     */
     ...
     * @example <b>参考示例：</b><br />
     *
     * myFontOverlay.addEventListener("mouseout", function(e) { alert(e.point); });
     *
     */

    /**
     * 为该覆盖物绑定一系列事件 当前支持click mouseover mouseout
     *
     * @return 无返回值。
     */
    FontOverlay.prototype._bind = function () {
        if (!this._domElement) {
            return;
        }
