...
(function () {
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        var pCount = 0;
        for (var i = 0, li = this._markers.length; i < li; i++) {
            pCount += parseInt(this._markers[i].getTitle());
        }
        this._clusterMarker.setText(pCount);

        var thatMap = this._map;
        var tZoom = thatMap.getZoom();
        //var thatBounds = this.getBounds();
        var thatMarkerClusterer = this._markerClusterer;
        this._clusterMarker.addEventListener("click", function (event) {
            if (thatMarkerClusterer) {
                thatMarkerClusterer.clearMarkers();
            }
            thatMap.centerAndZoom(event.point, tZoom + 1);
        });
