...
(function () {
     ...
     *
     */
    var _IMAGE_PATH = '/lib/images/gis/markerClusterer/m';
    ...
    var TextIconOverlay =
         ...
         * TextIconOverlay
         *
         * @class 此类表示地图上的一个覆盖物，该覆盖物由文字和图标组成，从Overlay继承。文字通常是数字（0-9）或字母（A-Z ），而文字与图标之间有一定的映射关系。 该覆盖物适用于以下类似的场景：需要在地图上添加一系列覆盖物，这些覆盖物之间用不同的图标和文字来区分，文字可能表示了该覆盖物的某一属性值，根据该文字和一定的映射关系，自动匹配相应颜色和大小的图标。
         *
         * @constructor
         * @param {Point}
         ...
         *            text 表示该覆盖物显示的文字信息。
         * @param {Json
         *            Object} options 可选参数，可选项包括：<br /> "<b>styles</b>":{Array<IconStyle>} 一组图标风格。单个图表风格包括以下几个属性：<br />
    ...
    /**
     * 由文字信息获取风格数组的对应索引值。 内部默认的对应函数为文字转换为数字除以10的结果，比如文字8返回索引0，文字25返回索引2. 如果需要自定义映射关系，请覆盖该函数。
     *
     * @param {String}
     *            text 文字。
     * @param {Array
     *            <IconStyle>} styles 一组图标风格。
